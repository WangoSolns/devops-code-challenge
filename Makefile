...
test-2-health: ## run pod health test
	...
	if kubectl -n wordpress get pods -o=jsonpath='{.items[*].status.phase}' | grep -qvx Running; \
	then \
...
test-3-curl: ## run curl request test
	...
	if curl --fail $(shell minikube service wordpress -n wordpress --url); \
	then \
...
test-4-probes: ## run pod probe test
	...
	( \
		export IMAGE_COUNT="$(shell kubectl -n wordpress get pods -o=jsonpath='{range .items[*].spec.containers[*].image}{.}{"\n"}{end}' | wc -l | tr -d ' ')"; \
		export LIVENESS_COUNT="$(shell kubectl -n wordpress get pods -o=jsonpath='{range .items[*].spec.containers[*].livenessProbe}{.}{"\n"}{end}' | wc -l | tr -d ' ')"; \
		export READINESS_COUNT="$(shell kubectl -n wordpress get pods -o=jsonpath='{range .items[*].spec.containers[*].readinessProbe}{.}{"\n"}{end}' | wc -l | tr -d ' ')"; \
...
test-5-endpoints:
	...
	if kubectl -n wordpress describe svc | grep Endpoints | grep -q '<none>'; \
	then \
